...
export function registerTagCommand(program: Command, container: Container): void {
  program
    ...
    .action(async (titleOrId: string, tagName: string | undefined, opts: { remove?: boolean; list?: boolean }) => {
      ...
      if (opts.remove) {
        ...
        await container.noteRepository.removeTagFromNote(note.id, tag.id);
        console.log(`Removed ${chalk.cyan(`#${tagName}`)} from "${note.title}"`);
      } else {
        await container.noteRepository.addTagToNote(note.id, tagName);
